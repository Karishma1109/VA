...
Loading the excel file "Superstore-2021.xls" using the function *read_xls* from the **readxl** module. The excel file contains several sheets, for reading worksheets, the sheet names can be specified in the *sheet* argument in the *read_xls* function. The work sheets "Orders" and "Returns" are loaded into data frames "orders" and "returns" respectively.
...
Let us now sort the the data in the "freq_returned" data frame by the number of returns.
...
Now, to create a pareto chart, we need to get the cumulative returns frequency for each sub-category of product. The cumulative returns frequency is further used to calculate the cumulative returns frequency as a percentage of total cumulative returns frequency.
...
The following **ggplot2** functions are used to create the standard pareto chart:
...
These functions are added to the *ggplot2* layout layer by layer as shown in the folded code.
Other functions, *labs* and *theme* are used and their arguments are modified to create the plot same as the one sketched above.
...
This visualization uses variables from the data set taken from the link. The data set is loaded as follows:
...
To get the population organized separately for Males and Females in various age groups, we will group by the data frame by "AG" and "Sex" fields and take the sum of "Population".
...
To create the population pyramid, horizontal bar charts are used to visualize the population of males and females into various age groups and aligning them on the same central axis. To align the central axis, transformation of the population of males variable is done as negative and then labelled as their original values in the plot.
...
1. *geom_bar*: 2 geom_bars are used to create bars of population values for Males and Females separately for the corresponding age groups.
2. *coord_flip*: Used to flip the coordinate axes and transform the default orientation of bars from vertical to horizontal.

Along with the above 2 functions, *scale_x_continuous*, *scale_y_continuous*, *labs* and *theme* functions are used to change the color, labels and titles of the plot.
...
Distill for RMarkdown [Link](https://rstudio.github.io/distill/basics.html)

ggplot2 documentation [Link](https://ggplot2.tidyverse.org/reference/)
