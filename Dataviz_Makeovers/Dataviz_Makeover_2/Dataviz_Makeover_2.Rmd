...
The [given](https://public.tableau.com/app/profile/tskam/viz/prototype1_16468762782120/Dashboard1) interactive data visualization is prepared using data from the following sources:
...
The "Year Month" variable has only one unique value "2022-01" which means the bus-trips mentioned in the data set are for the month of January 2022. Other fields in the data set were not used to prepare given and the proposed data visualizations. 
...
![*Figure 3: Complete Field Name highlighted in red box*](images/image2.jpg){width=50%}


The field name is "Weekend/Holidays" is not fully captured in the data visualization leading to lack of clarity on what the field refers to. In this case, the users might confuse bus-trips that take place on a holiday that falls on a weekday to be counted in the "WEEKDAY" plots leading to misinterpretation. This is because they cannot see that the weekends and holidays are actually combined and represented separately in the "WEEKDAYS.." plots.

### 4.1.3 Incomplete Plot for Weekend/Holidays Adjacency Matrix of bus-trips between Origin and Destination sub-zones.

The number of origin and destination sub-zones are the same for the weekday adjacency matrix and the weekends/holidays adjacency matrix and the aspect ratio is also the same but still the size of the adjacency matrix of the weekdays/holidays for bus-trips between origin and destination sub-zones is smaller, meaning that the plot is incomplete. The figure 4 represents the incomplete plot. Ideally, the entire view of all the plots in a dashboard should be captured in one screen. However, there is no scroll bar also to scroll and see the full visual. The users can not actually make any meaningful conclusion because of incompleteness and lack of clarity.

![*Figure 4: Incomplete Weekend Adjacency Matrix*](images/image3.jpg){width=40%}

### 4.1.4 Lack of Clarity in the Adjacency Matrix plots
  ...
  c) **Lack of clarity in the Tool tips**: 
  
### 4.1.5 Existence of NULL Values for the Origin and Destination sub-zones
...
The sizes of the weekday and weekends/holidays adjacency matrices are different, the weekday one is bigger and the weekends/holidays one is smaller which looks inappropriate and degrades the aesthetics of the dashboard.

### 4.2.6 The size of the adjacency matrices is too small

The size of the adjacency matrix is too small for the users to different one point from the other.
...
# 5. Proposed Interactive Data Visualization Dashboard
...
3. **To & Fro Tab**: This tab enables users to compare the number of trips originating from one or multiple sub-zones of their choice for weekday and weekdays/holidays with each other and with the number of trips attracted one or multiple sub-zones for weekday and weekdays/holidays. They can also choose the time per hour of their choice by moving the given slider tab. This allows users to view the frequency of bus-trips originating from and attracted to various sub-zones and compare them in several ways:
  ...
  a. Compare number of bus-trips originating from one sub-zone with one or more other sub-zones.
    
  b. Compare number of bus-trips attracted to one sub-zone with one or more other sub-zones.
    
  c. Compare the weekday and weekends/holiday frequencies for the cases a & b mentioned above and across these two cases.
    
### 5.1 Tabs In-Bound and Out-Bound
...
![*Figure 12: In-Bound and Out-Bound tabs"*](images/Tab1.jpeg){width=70%}


![*Figure 12: To & Fro Tab"*](images/Tab 3.jpeg){width=70%}


The final visualization can be found here- [Bus Flows in Singapore at Planning Sub-Zone Levels](https://public.tableau.com/app/profile/karishma.yadav2567/viz/BUS-TRIPSACROSSPLANNINGSUB-ZONESSINGAPORE/TotalBus-Trips?publish=yes).
...
1. **Clarity issues addressed by the proposed dashboard**: In terms of clarity, the proposed dashboard addresses the following issues:
  ...
  a. It has detailed titles for all the components including the dashboard, the tabs and the individual charts which improves clarity about the contents of the dashboard.
  
  b. It provides the users with clear directions on how to navigate through the dashboard by mentioning proper titles for the filters and legends. In the given visualization, the filters and labels are merged together, the users cannot identify how to interact with the dashboard.
  c. It removes null values to avoid ambiguity.
  
  d. It transforms the adjacency matrix into geo-spatial plots where users can choose the origin sub-zone of their choice and look at the number and percentage of bus-trips attracted to each of the destination sub-zones. The clarity is improved in the sense that, in the adjacency matrix, the users cannot directly find the origin sub-zone they want to see the % of total bus trips attracted to each of the destination sub-zones, they can neither easily navigate to the destination sub-zone they want to look at. The adjacency matrix tooltip also does not mention the number of trips, which would have been useful for the users. Geo-spatial plot with color legend for the number of trips and tool tips for the percentage of trips attracted to each destination makes the information easy to access and gives clarity on the number of trips as well. 
  
  e. The proposed design places each of the filter selections and legends for their respective plots to avoid any confusion.
...
2. **Aesthetics issues addressed by the proposed dashboard**: The aesthetics have been significantly improved in the proposed dashboard, in the following ways:
  ...
  a. The positioning of charts are aligned.
  
  b. The sizes and aspect ratios of the charts are aligned.
  
  c. The legends and filters are organized appropriately.
  
  d. The dashboard view fits into the screen.
  
  e. The weekday and weekends/holidays bus-trip frequencies are colored in different colors to make the plots easier to view and also look good.
  
  f. The number of sub-zone data points on the plot which were in a list format in the given visualization have been changed to drop-down menu to have better aesthetics.
  
  g. Tabs have been introduced which eliminate unnecessary cluttering of the dashboard view and make it easy to navigate and better to look at.
...
## 6. The Makeover Visualization

The final visualization can be found here- [Bus Flows in Singapore at Planning Sub-Zone Levels](https://public.tableau.com/app/profile/karishma.yadav2567/viz/BUS-TRIPSACROSSPLANNINGSUB-ZONESSINGAPORE/TotalBus-Trips?publish=yes).


### 6.1 Steps-by-Step Explaination
...
![*Figure 12: Exploring Null Values](images/image9.jpg){width=50%}

![*Figure 12: Exploring Null Values in the "Destination_Sz" field*](images/image10.jpg){width=90%}


![*Figure 12: Exploring Null Values in the "Origin_Sz" field*](images/image11.jpg){width=90%}
...
![*Figure 12: Removing Null values using filters*](images/image12.jpg){width=60%}

![*Figure 13: Removed Null values for Origin and Destination sub-zone fields*](images/image13.jpg){width=40%}
...
![*Figure 12: Hiding unnecessary fields*](images/image6.jpg){width=50%}
...
![*Figure 12: Creating the world map*](images/image14.jpg){width=50%}

Step 8: Added Geometry to the Details to get the sub-zone areas in Singapore as shown in the figure below.

![*Figure 12: Creating the Sub-Zone Geometry*](images/image15.jpg){width=50%}
...
![*Figure 12: Adding Color and Details*](images/image16.jpg){width=50%}
...
![*Figure 12: Adding filters*](images/image17.jpg){width=50%}
...
![*Figure 12: Show filters*](images/image18.jpg){width=40%}


Step 12: Filter types for the "Day Type" and "Destination_Sz" were changed as shown in the figure below.

![*Figure 12: Change filters*](images/image20.jpg){width=40%}
...
![*Figure 12: Edit Title*](images/image21.jpg){width=40%}

![*Figure 12: Changing the filter Title Text*](images/image22.jpg){width=40%}
...
![*Figure 12: Changing the Sheet Title Text*](images/image23.jpg){width=70%}
...
![*Figure 12: Creating % of total trips field*](images/image24.jpg){width=50%}
...
![*Figure 12: Editing tooltip details*](images/image25.jpg){width=50%}

![*Figure 12: Frequency of Bus-Trips from Origin sub-zones for the Selected Destination*](images/image26.jpg){width=50%}
...
![*Figure 12: Bus Trips Originating from Selected Origin Sub-Zone by Day Type and Time Per Hour*](images/image27.jpg){width=50%}
...
![*Figure 12: Moving fields to rows and columns*](images/image29.jpg){width=50%}
...
![*Figure 12: Adding filters and colors*](images/image28.jpg){width=50%}
...
![*Figure 12: Setting the ticks of "Time Per Hour axis"*](images/image30.jpg){width=50%}
...
![*Figure 12: Bus-Trips Originating from selected origin sub-zone/s by time per hour"*](images/image31.jpg){width=70%}

Step 20 was followed to create the same chart for "Destination_Sz" in the second workbook.

Step 21: Once the 3 sheets were created in the 2 workbooks for the Origin and Destination Sub-Zones respectively, dashboards were created.

Step 22: The dashboards "In-Bound Traffic to Destination Sub-Zone", "Out-Bound Traffic From Origin Sub-Zone", "Trip Frequency Map for Origin Sub-Zone" and "Trip Frequency Map for Destination Sub-Zone" were published on Tableau public.

Step 23: An embedded web-page link was created for the "Trip Frequency Map for Origin Sub-Zone" dashboard by adding "&:toolbar=no&:showVizHome=no&embed=true" extension to the dashboard share link on Tableau. This link was used to add this as a webpage object to the "Trip Frequency Map for Destination Sub-Zone" dashboard to get them side by side as shown in the figure below.

![*Figure 12: Bus-Trips across Planning sub-zones in Singapore by Day Type"*](images/image32.jpg){width=70%}

Step 24: Step 23 was repeated to create a tableau embed link for the "Out-Bound Traffic From Origin Sub-Zone" dashboard and this was pasted in the workbook 1 which has the "In-Bound Traffic to Destination Sub-Zone" dashboard. Along with it the dashboard "Bus-Trips across Planning sub-zones in Singapore by Day Type" created in step 23 was also added as another dashboard by copy and pasting the dashboard across sheets.
...
![*Figure 12: Adding Navigation Buttons to the Dashboards"*](images/image33.jpg){width=70%}


# 6. Observations:

1.	The morning and evening peak of inbound and outbound bus trips respectively is also an indication of the workforce commuting to and from the airport.

2.	Incase of Sentosa an interesting trend has been observed where the number of inbound trips on weekend around 11 am is same as on weekdays. This tells us that the place is a popular spot for recreation. 

3. Tampines east appears to be the most popular sub-zone, from where majority of the buses originate as well as terminate. Also majority of the buses that start at Tampines east also terminate at Tampines east
